...
| answered | 1.651 | no pair separates |

*(Corrected 2026-08-12: the `answered` cell read **1.174** from this page's first version, and
CLAUDE.md, README.md, finding 16 and `TODO.txt` all copied it. Recomputed with
`scripts/analyze_rank_metrics.py` on `results_p5` — the tree this table was built from — and on
`results_p40`, which agree exactly, the six `answered` t-values are 0.808, 0.966, 1.174, 0.279,
**1.651**, 0.754. **1.174 is the largest among the three BioSentVec pairs only**, not across all
six as this column's header says; the true maximum is Bio_ClinicalBERT vs BlueBERT at 1.651. The
verdict column is unchanged — 1.651 is still well under 2.262, and no pair separates on any
population. Note the `all-cases` and `answered` maxima are the **same number for a reason**: the
three BERT-only pairs are bit-identical between those two blocks, because the BERT arms never
abstain and so lose no cases when abstentions are excluded. Only the BioSentVec pairs differ, and
under this correction none of them is the maximum.)*
...
- ~~**Per-case output** (`docs/findings/10`) blocks the self-selection test above. Highest-value
  item this finding produces.~~ **Delivered 2026-08-12 as `RankCases.txt` (P40)** — a new sibling
  file rather than a repair of the dead handles — and the self-selection test it unblocked is
  [finding 16](16-self-selection.md). The answer: the baseline's 98 answered cases really are
  easier, by +0.032 to +0.038 pooled MRR for *every* arm, so the confound is real but arm-neutral;
  on the matched 98-case set the ordering inverts and `answered`'s baseline-first result is
  explained as a mismatched denominator.
- ~~**P39** — the two arms break score ties differently (`list.sort` is stable, `np.argsort` is
  ...
  bounding the MRR shift at 0.0022. Filed, not fixed.~~ **Closed 2026-08-12, measured-moot, with
  no code change.** The bound above was a StubEncoder inference; measuring both arms directly
  gives a tie-permutation envelope of **exactly 0.000000** on all three populations, for MRR and
  Hit@10, macro and pooled. The counts here describe the stub configuration and remain correct for
  it — the real-encoder census is different (baseline: 1 tied pair in 129 cases; Bio_ClinicalBERT:
  31 pairs across 30 cases) and reaches the same span.
